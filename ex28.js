...
function calcularDiagonais(matriz10) {
  ...
  for (let i = 0; i < matriz10.length; i++) {
    for (let j = 0; j < matriz10[i].length; j++) {
      if (j > i) {
        somaAcimaDiagonal += matriz10[i][j];
      } else if (j < i) {
        somaAbaixoDiagonal += matriz10[i][j];
      }
    }
  }
